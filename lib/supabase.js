...
function createDb() {
  ...
  return {
    ...
    async insert(tableObj, row) {
      const { data, error } = await supabase.from(tableObj._name).insert(toSnake(row)).select().single();
    ...
    update(tableObj) {
      ...
      return {
        ...
        async set(values) {
          let query = supabase.from(tableObj._name).update(toSnake(values)).select();
          for (const [f, v] of filters) query = query.eq(f, v);
